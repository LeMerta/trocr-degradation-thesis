"""
generate_degraded_combo_test_iam.py

Loads the IAM test set from HuggingFace, applies multiple degradation methods combined
in different intensity levels, and uploads the results as parquet files to HF.

Each degraded set is saved under:
    iam/combined_degradations/test/combo_<1,2,.. ,5>.parquet
"""
...
for i, intensities in enumerate(DEGRADATIONS):
    ...
    target_path = f"iam/combined_degradations/test/combo_{i + 1}.parquet"
