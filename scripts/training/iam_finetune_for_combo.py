"""
iam_finetune_for_combo.py

Finetunes the base IAM TrOCR model on IAM training set with combined and mixed degradations.
...
for combo_name in COMBINATIONS:
    ...
    train_dataset = load_dataset(
        "parquet",
        data_files=f"hf://datasets/{HF_REPO_ID}/iam/combined_degradations/train/{combo_name}_train.parquet",
        split="train",
    )
    val_dataset = load_dataset(
        "parquet",
        data_files=f"hf://datasets/{HF_REPO_ID}/iam/combined_degradations/train/{combo_name}_val.parquet",
        split="train",
    )
    ...
    # Save model locally
    save_path = f"/data/{os.environ['USER']}/temp_model/iam_{combo_name}"
    ...
    api.upload_folder(
        ...
        repo_type="model",
        path_in_repo=f"iam_models/iam_{combo_name}",
    )
