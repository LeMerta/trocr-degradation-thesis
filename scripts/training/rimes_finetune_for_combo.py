"""
rimes_finetune_for_combo.py

Finetunes the base RIMES TrOCR model on RIMES training set with combined and mixed degradations.
At the end the checkpoint with the lowest validation CER is uploaded to Hugging Face
under LeMerta/finetuned-trocr-models in the rimes_models directory.
...
for combo_name in COMBINATIONS:
    print(f"Training for: combo={combo_name} ")

    # Load datasets
    print("Loading datasets...")
    train_dataset = load_dataset(
        "parquet",
        data_files=f"hf://datasets/{HF_REPO_ID}/rimes/combined_degradations/train/{combo_name}_train.parquet",
        split="train",
    )
    val_dataset = load_dataset(
        "parquet",
        data_files=f"hf://datasets/{HF_REPO_ID}/rimes/combined_degradations/train/{combo_name}_val.parquet",
        split="train",
    )
    ...
    trainer.train()
    print(
        f"Best val CER for {combo_name}: {trainer.state.best_metric:.4f}"
    )

    # Save model locally
    save_path = f"/data/{os.environ['USER']}/temp_model/rimes_{combo_name}"
    ...
    api.upload_folder(
        ...
        repo_type="model",
        path_in_repo=f"RIMES_models/RIMES_{combo_name}",
    )
