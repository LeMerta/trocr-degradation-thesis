...
Finetunes the RIMES-base model on degraded RIMES training sets stored in Hugging Face.
...
for method_name, intensity in DEGRADATIONS:
    ...
    train_dataset = load_dataset(
        "parquet",
        data_files=f"hf://datasets/{HF_REPO_ID}/rimes/{method_name}/train/{method_name}_{intensity}_train.parquet",
        split="train",
    )
    val_dataset = load_dataset(
        "parquet",
        data_files=f"hf://datasets/{HF_REPO_ID}/rimes/{method_name}/train/{method_name}_{intensity}_val.parquet",
        split="train",
    )
    ...
    # Save model locally
    save_path = f"/data/{os.environ['USER']}/temp_model/rimes_{method_name}"
    ...
    api.upload_folder(
        ...
        repo_type="model",
        path_in_repo=f"RIMES_models/RIMES_{method_name}",
    )
